...
public class RadioButtonItemEventEx extends JFrame {
    ...
    private ImageIcon[] image = {new ImageIcon("images/apple.jpg"), new ImageIcon("images/pear.jpg"), new ImageIcon("images/cherry.jpg")};
    ...
    public RadioButtonItemEventEx() {
        ...
        c.add(radioPanel, BorderLayout.NORTH); // 컨텐트팬의 NORTH에 라디오 패널 부착
    ...
    class MyItemListener implements ItemListener {
        public void itemStateChanged(ItemEvent e) {
            if (e.getStateChange() == ItemEvent.DESELECTED) return; //해제된 경우 그냥 리턴 
            if (radio[0].isSelected()) imageLabel.setIcon(image[0]); // 사과가 선택된 경우
            else if (radio[1].isSelected()) imageLabel.setIcon(image[1]); // 배가 선택된 경우
            else imageLabel.setIcon(image[2]); // 체리가 선택된 경우
        }
    }
